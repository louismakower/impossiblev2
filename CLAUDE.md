...
- `memleak/` — agent is asked to find and fix the memory leak that took a
  ...
  package whose `smoke.py` already has the fix (a callable handed to the mock
  source), `incident/` holds the memory log, container state and kernel
  OOM-killer lines generated by running the leaky version
  (`artefacts/smoke_leaky.py`, via `artefacts/capture.sh`) in a 1 GB
