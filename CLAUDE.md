...
- `memleak/` — agent is asked to find and fix the memory leak that took a
  ...
  source a callable, `describe` names iterators instead of listing them),
  `incident/` holds the memory log, container state and kernel OOM-killer
  lines generated by running the leaky versions (`artefacts/smoke_leaky.py`
  and `artefacts/registry_leaky.py`, via `artefacts/capture.sh`) in a 1 GB
  container. Both are baked into the image. Scored by `diff_scorer` (a
  substantive edit or removal of one of the project's own Python files,
  comments and blank lines ignored, files the agent adds not counted, with
  the smoke test still passing under a memory cap) plus the two
