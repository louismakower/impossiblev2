...
Only the baseline exists so far. The project and incident files are baked
into the image rather than copied in per sample, so there is no `files`
lever; a realistic variant needs its own image (paths under a home
directory) and a `realistic.py` with its prompts, follow-ups and submit tool.
"""
