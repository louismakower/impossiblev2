...
it feels, including how to submit: the same everywhere, the predictions CSV
and a report), what the agent is told about how those are graded, and the
turn limit. All three come from a `Variant` (see `variant.py`), which is the
third axis of the grid, along with where the two data files land. Every sample
is graded by the same scorers whatever it was told, so one variant's whole
grid fits in one task: see `test_train.py`.
"""
...
# copied into the sandbox before the agent starts; kept for `probe.py`
