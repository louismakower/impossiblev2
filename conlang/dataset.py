...
shared turn-limit sentence.

The data is mounted into the sandbox by `compose.yaml`, so the sample carries
no files. Metadata names the grid cell the way the other tasks do, so
analysis code can group by it once more cells exist.
"""
